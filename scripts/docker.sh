...
# Scholar AI Gap Analyzer Docker Management Script
# This script provides commands to build, run, test, and manage the Docker containerized Gap Analyzer
...
# Configuration
APP_NAME="scholar-gap-analyzer"
CONTAINER_NAME="scholar-gap-analyzer"
IMAGE_NAME="scholar-gap-analyzer:latest"
...
run() {
    ...
    if docker ps | grep -q "$CONTAINER_NAME"; then
        ...
        print_status "Health check endpoint: http://localhost:$DEFAULT_PORT/health"
    else
...
test() {
    print_status "Testing $APP_NAME service..."
    ...
    if curl -f http://localhost:$DEFAULT_PORT/health; then
...
show_help() {
    echo -e "${CYAN}Scholar AI Gap Analyzer Docker Management Script${NC}"
    echo ""
